...
export function renderGuide(pagePath: string): string {
...
<main>
  <div class="gpanel">
    ...
    <ul>
      ...
      <li data-l="zh"><strong>右上角</strong>：⚙ 打开设置卡（改日记目录、从历史生成画像）；? 打开本指南。</li>
      <li data-l="en"><strong>Top right</strong>: ⚙ opens the settings card (change the diary directory, generate the profile from history); ? opens this guide.</li>
    </ul>
    ...
    <h2><span data-l="zh">长期记忆：用户画像</span><span data-l="en">Long-term memory: the user profile</span></h2>
    <ul>
      <li data-l="zh"><strong>它是什么</strong>：每次提交后，系统在评注之外再维护一份关于你的画像——纯文本 markdown，存在日记目录隐藏文件夹的 <code>.diary-meta/profile.md</code>，可以直接翻开看。分三层：人格核心（跨场景稳定的价值观与认知特点）、阶段性状态（当前阶段的执念与项目）、近期事件（当天及最近的具体事件，会被自然淘汰）。</li>
      <li data-l="en"><strong>What it is</strong>: alongside the comment, each submit maintains a profile about you — plain markdown at <code>.diary-meta/profile.md</code> in the diary directory's hidden folder, readable at any time. Three layers: core self (stable values and cognitive traits across situations), current phase (this phase's obsessions and projects), and recent events (the day's concrete events, which age out naturally).</li>
      <li data-l="zh"><strong>怎么参与总结</strong>：生成当日总结前，画像全文与近一周记录日的概要（每日关键词 + 一句话，从往日评注块提取，零额外调用）作为参考拼进请求；当天的日记全文始终是唯一主体。没有画像时总结照常，只是少了这段参考。</li>
      <li data-l="en"><strong>How it feeds the summary</strong>: before generating the day's summary, the profile plus a digest of the last week's recorded days (each day's keyword + one-liner, extracted from past comment blocks, zero extra calls) is appended to the request as reference; the day's full text remains the only subject. With no profile the summary proceeds normally, just without that reference.</li>
      <li data-l="zh"><strong>失败不影响保存</strong>：画像更新失败时保存与总结照常成功，页面只提示「画像更新失败，不影响保存」；没有重试按钮，下次提交天然自愈。日记原文永远不被「遗忘」改动——画像是可重算的派生状态。</li>
      <li data-l="en"><strong>Failures never block saving</strong>: when the profile update fails, saving and the summary still succeed; the page only notes "profile update failed, saving unaffected". There is no retry button — the next submit self-heals. Your diary text is never modified by "forgetting": the profile is recomputable derived state.</li>
      <li data-l="zh"><strong>从历史生成画像</strong>：设置卡里的「从历史生成画像」按钮，只在检测不到画像文件时可点（有画像即禁用，永不覆盖维护中的画像）；点击后弹成本警告并二次确认，从全部历史日记按时间顺序分块生成。换机器 = 拷贝 <code>.diary-meta/profile.md</code>。</li>
      <li data-l="en"><strong>Generate from history</strong>: the "Generate profile from history" button in the settings card is only clickable when no profile file is detected (disabled once one exists — it never overwrites a profile under maintenance); clicking shows a cost warning and asks for a second confirmation, then rebuilds from your entire history in chronological chunks. Moving machines = copy <code>.diary-meta/profile.md</code>.</li>
    </ul>
    ...
    <ul>
      <li data-l="zh"><strong>数据全在日记目录里</strong>：正文是 <code>YYYY-MM-DD.md</code>，热力图 emoji 在同目录的隐藏文件夹 <code>.diary-meta/</code>（一年一个 JSON），用户画像是同文件夹的 <code>profile.md</code>。备份 = 拷贝整个目录。</li>
      <li data-l="en"><strong>Everything lives in the diary directory</strong>: entries are <code>YYYY-MM-DD.md</code> files; heatmap emoji live in the hidden <code>.diary-meta/</code> folder inside the same directory (one JSON per year), and the user profile is the <code>profile.md</code> next to them. Backup = copy the whole directory.</li>
      <li data-l="zh"><strong>更换目录</strong>：页面设置卡改路径后，把旧目录整体搬过去即可——别忘了隐藏的 <code>.diary-meta/</code>（访达里按 ⌘⇧. 显示隐藏项）。只拷 md 文件不拷 meta 的话，热力图会退化成无 emoji 的绿块、画像回到冷启动（都不报错）。</li>
      <li data-l="en"><strong>Changing the directory</strong>: update the path in the settings card, then move the whole directory — do not forget the hidden <code>.diary-meta/</code> (in Finder press ⌘⇧. to reveal hidden items). Copying only the .md files loses emoji silently (the heatmap degrades to plain green cells) and resets the profile to a cold start (both without an error).</li>
    ...
    <p data-l="zh">日记全文只落在<strong>你本机的目录</strong>里。外发的调用有两次：「生成总结」会把当日全文连同用户画像、近一周日记概要一起发给你配置（或当次选择）的 LLM provider；「更新画像」把旧画像与当日全文发去换一份新画像。热力图数据、元表、草稿都不离开本机（草稿在浏览器 localStorage）。</p>
    <p data-l="en">Your full diary text is stored only <strong>on your machine</strong>. Two kinds of calls go out: "generate summary" sends the day's full text together with the user profile and last week's digest to the configured (or selected) LLM provider, and "update profile" sends the old profile plus the day's full text to get a new one. Heatmap data, meta files and drafts never leave your machine (drafts live in browser localStorage).</p>
