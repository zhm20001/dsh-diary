  ...
  <div class="gpanel">
    ...
    <table>
      ...
      <tr>
        ...
        <td data-l="zh">页面设置卡写的就是它；也可手改。字段：<code>diaryDir</code>、<code>templatePath</code>（支持 <code>~/</code>）。改后即时生效（diaryDir 每次请求现读）。</td>
        <td data-l="en">What the settings card writes; also hand-editable. Fields: <code>diaryDir</code>, <code>templatePath</code> (both accept <code>~/</code>). diaryDir is re-read on every request, so edits apply immediately.</td>
      </tr>
      <tr>
