...
export function loadPathVars(): PathVars {
  let raw: unknown
  try {
 ...
 */
export async function savePathVars(patch: Partial<PathVars>, targetPath = resolve(PLUGIN_ROOT, 'config.json')): Promise<void> {
  ...
  for (const key of Object.keys(patch) as (keyof PathVars)[]) {
