...
import { buildCommentBlock, diaryPathFor, extractRecordDate, fillTemplate, framesFor, joinBlock, thirtyHour, withoutTrailingCommentBlock, } from "./core.js";
import { BUNDLED_TEMPLATE, absolutize, loadConfigVars, resolveDirValue, saveConfigVars } from "./paths.js";
import { renderGuide } from "./guide.js";
import { renderPage } from "./page.js";
import { DEFAULT_SUMMARY_PROMPT, promptDefects, runSummary } from "./summary.js";
import { buildSummaryUserMessage, formatDigestLine, memoryWindow, parseDayDigest, profileExists, readProfileText, rebuildProfile, runProfileUpdate, } from "./profile.js";
/**
 ...
 */
function webCarrier(ctx) {
    const c = ctx;
    const carrier = c.webServer ?? c.httpServer;
    if (!carrier)
        throw new Error('dsh-diary: 宿主未提供 webServer/httpServer 路由服务（需要 dsh web 宿主）');
    return carrier;
}
export class DiaryService extends Service {
    ...
    constructor(ctx, config) {
        ...
        ctx.effect(() => web.register({
            ...
            handler: (req, res) => {
                this.handleApi(req, res).catch((err) => {
    ...
    async handleApi(req, res) {
        ...
        try {
            if (req.method === 'GET' && path === `${this.config.pagePath}/api/today`)
                return await this.handleToday(res);
            if (req.method === 'GET' && path === `${this.config.pagePath}/api/dates`)
                return await this.handleDates(res);
            if (req.method === 'GET' && path === `${this.config.pagePath}/api/models`)
                return await this.handleModels(res);
            if (path === `${this.config.pagePath}/api/settings`)
                return await this.handleSettings(req, res);
            if (path === `${this.config.pagePath}/api/prompt`)
                return await this.handlePrompt(req, res);
            if (req.method === 'POST' && path === `${this.config.pagePath}/api/submit`)
                return await this.handleSubmit(req, res);
            if (req.method === 'POST' && path === `${this.config.pagePath}/api/retry-summary`)
                return await this.handleRetry(req, res);
            if (path === `${this.config.pagePath}/api/rebuild-profile`)
                return await this.handleRebuildProfile(req, res);
    ...
    async handleDates(res) {
        ...
        const dates = [...new Set(names.map(extractRecordDate).filter((d) => d !== null))].sort();
    ...
    async readDayMetas(dir) {
        ...
        for (const name of names) {
            if (!/^\d{4}\.json$/.test(name))
                continue;
            try {
                const raw = JSON.parse(await readFile(resolve(this.metaDirOf(dir), name), 'utf8'));
                if (typeof raw !== 'object' || raw === null)
                    continue;
                for (const [date, value] of Object.entries(raw)) {
                    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || typeof value !== 'object' || value === null)
                        continue;
    ...
    async saveDayEmoji(dir, date, emoji) {
     ...
     */
    async resolveModelOverride(body) {
    ...
    trustedHosts() {
        ...
        const runtime = this.pluginCtx.get('webRuntime');
    ...
    async handleSubmit(req, res) {
        ...
        try {
            ...
            if (summary.emoji !== undefined)
                await this.saveDayEmoji(dir, date, summary.emoji);
     ...
     */
    async updateProfile(dir, diaryText, override) {
        try {
     ...
     */
    async handleRebuildProfile(req, res) {
        if (req.method !== 'POST') {
    ...
    async readDayDigest(dir, files, date) {
    ...
    async historyEntries(dir) {
        ...
        for (const [date, names] of [...byDate.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1))) {
            ...
            if (texts.length > 0)
                entries.push({ date, text: texts.join('\n\n') });
        }
        return entries;
    }
    async summarize(dir, date, diaryText, override = {}) {
 ...
 */
function isLocalRequest(req, trustedHosts) {
