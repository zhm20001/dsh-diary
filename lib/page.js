...
export function renderPage(apiPath) {
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .profileRow { display: flex; gap: 10px; align-items: center; margin-top: 14px; flex-wrap: wrap; }
  .profileRow .dirNote { flex: 1 1 100%; margin-top: 0; }
  ...
  .warn { margin-top: 12px; font-size: 12px; line-height: 1.7; color: var(--err-text); }
...
<main>
  <div class="panel setup" id="setupPanel">
    ...
    <div class="profileRow">
      <button class="btn ghost" id="rebuildBtn" type="button" disabled>从历史生成画像</button>
      <span class="dirNote" id="rebuildNote"></span>
    </div>
    <p class="setupHint">长期记忆：每次提交后系统自动更新一份用户画像（纯文本，存在日记目录 <code>.diary-meta/profile.md</code>），之后的总结会结合它和近一周日记概要作参考；更新失败不影响保存。还没有画像时，可用全部历史日记生成一份——会发起多次调用、消耗较多上下文用量。</p>
  ...
  <div class="panel">
    ...
    <div class="panel result" id="card">
      ...
      <div class="warn" id="pwarn" hidden></div>
...
(function () {
  ...
  // 「从历史生成画像」守卫态：检测到画像文件即禁用（永不覆盖维护中的画像）
  function rebuildState(hasProfile) {
    rebuildBtn.disabled = !!hasProfile;
    rebuildNote.className = 'dirNote';
    rebuildNote.textContent = hasProfile ? '已存在用户画像，全量重算不可用' : '';
  }

  rebuildBtn.addEventListener('click', function () {
    if (!window.confirm('将从全部历史日记按时间顺序分块生成用户画像，期间发起多次 LLM 调用、消耗大量上下文用量。仅在没有画像时可执行。确定继续？')) return;
    rebuildBtn.disabled = true;
    rebuildNote.className = 'dirNote';
    rebuildNote.textContent = '生成中……历史越长耗时越久，请勿关闭页面';
    fetch(API + '/api/rebuild-profile', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })
      .then(parseJson)
      .then(function (j) {
        if (j.ok) {
          rebuildNote.textContent = '已生成用户画像（.diary-meta/profile.md），之后的总结会自动参考它';
          rebuildBtn.disabled = true;
        } else {
          rebuildNote.className = 'dirNote warn';
          rebuildNote.textContent = j.error || '生成失败';
          rebuildBtn.disabled = false;
        }
      })
      .catch(function (e) {
        rebuildNote.className = 'dirNote warn';
        rebuildNote.textContent = '生成失败：' + String(e);
        rebuildBtn.disabled = false;
      });
  });
  ...
  function showResult(j) {
    ...
    if (j.warning) { pwarn.hidden = false; pwarn.textContent = j.warning; } else { pwarn.hidden = true; }
