...
const REQUIRED_MARKERS = ['【今日关键词】', '【一句话总结】', '【评注】'];
/**
 * 校验自定义评注 prompt，返回问题列表（空数组 = 合法）。
 * 标记按“包含”匹配：不要求行首、不查重复——解析器取模型输出的首个匹配，
 * prompt 里标记出现两次只是啰嗦不是错误。空与超长按去首尾空白后计。
 */
export function promptDefects(prompt) {
    const defects = [];
    const text = prompt.trim();
    if (text.length === 0)
        defects.push('prompt 不能为空');
    for (const marker of REQUIRED_MARKERS) {
        if (!text.includes(marker))
            defects.push(`缺少必需标记${marker}`);
    }
    if (text.length > SUMMARY_PROMPT_MAX)
        defects.push(`过长（${text.length} > ${SUMMARY_PROMPT_MAX} 字符）`);
    return defects;
}
 ...
 */
export async function runSummary(ctx, cfg, userText) {
    const messages = [
        createUserMessage({
