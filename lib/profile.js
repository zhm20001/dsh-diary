...
import { finishError, textBlocksContent } from "./llm.js";
import { COMMENT_HEADING } from "./core.js";
 ...
 */
export async function readProfileText(diaryDir) {
    try {
 ...
 */
export async function writeProfileText(diaryDir, text) {
...
export function buildUpdateInput(oldProfile, diaryText) {
 ...
 */
export function parseDayDigest(content) {
...
export function formatDigestLine(date, digest, emoji) {
 ...
 */
export function memoryWindow(recordDates, today, windowDays = MEMORY_WINDOW_DAYS) {
 ...
 */
export function buildSummaryUserMessage(diaryText, memory) {
    const sections = [];
    if (memory.profileText.trim().length > 0)
        sections.push(`【用户画像】\n${memory.profileText.trim()}`);
    if (memory.digestLines.length > 0)
        sections.push(`【近期日记概要】\n${memory.digestLines.join('\n')}`);
 ...
 */
export async function generateProfile(ctx, cfg, oldProfile, diaryText) {
    const messages = [
        createUserMessage({
    ...
    if (text.trim().length === 0)
        throw new Error('diary: 画像更新输出为空');
    return text;
}
/**
 ...
 */
export async function runProfileUpdate(ctx, cfg, diaryDir, oldProfile, diaryText) {
 ...
 */
export async function rebuildProfile(ctx, cfg, diaryDir, entries) {
    let profile = '';
